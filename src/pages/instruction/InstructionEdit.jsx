...
const InstructionEdit = () => {
  ...
  useEffect(() => {
    if (currentInstruction) {
      ...
      setFormData({
        ...
        dueDate: currentInstruction.dueDate || '',
        assignedTo: currentInstruction.assignedTo || '',
        budget: currentInstruction.budget ? String(currentInstruction.budget) : ''
      })
    }
  }, [currentInstruction])
  ...
  if (isLoadingInstruction && !currentInstruction) {
    return (
  ...
  if (fetchError) {
    return (
      <div className="bg-red-100 text-red-700 p-4 rounded-md">
        {fetchError instanceof Error ? fetchError.message : '데이터를 불러오는 중 오류가 발생했습니다.'}
      </div>
    )
  }
      ...
      <Card className="bg-white shadow-md rounded-lg p-6">
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end mt-6">
            ...
            >
              {updateInstructionMutation.isLoading ? '저장 중...' : '변경사항 저장'}
            </Button>
          </div>
        </form>
      </Card>
