...
const WorkEdit = () => {
  ...
  const { useWorkDetail, useUpdateWork } = useWorkQueries()
  ...
  useEffect(() => {
    if (currentWork) {
      ...
      setFormData({
        ...
        completionRate: currentWork.completionRate ? String(currentWork.completionRate) : ''
      })
    }
  }, [currentWork])
  ...
  const validateForm = () => {
    ...
    if (formData.completionRate && (isNaN(Number(formData.completionRate)) || Number(formData.completionRate) < 0 || Number(formData.completionRate) > 100)) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const workData = {
        ...formData,
        completionRate: formData.completionRate ? Number(formData.completionRate) : 0
      }
  ...
  if (workError) {
    return (
      <div className="bg-red-100 text-red-700 p-4 rounded-md">
        {workError instanceof Error ? workError.message : '데이터를 불러오는 중 오류가 발생했습니다.'}
      </div>
    )
  }
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end mt-6">
            <Button
              type="submit"
              variant="primary"
              disabled={updateWorkMutation.isLoading || isLoadingInstructions}
