    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <Card className="lg:col-span-2 bg-white shadow-md rounded-lg p-6">
          <div className="mb-4">
            <h2 className="text-xl font-bold mb-2 text-gray-800">{currentWork.name}</h2>
            <div className="flex items-center space-x-4 mb-4">
              <span className="text-gray-500">ID: {currentWork.id}</span>
              <span className={`px-2 py-1 text-xs rounded-full ${statusOptions.find(s => s.value === currentWork.status)?.color || 'bg-gray-100'}`}>
                {currentWork.status}
              </span>
            </div>
            <p className="text-gray-700 mb-4">{currentWork.description}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-gray-800">₩{currentWork.cost?.toLocaleString() || '0'}</p>
            </div>
            <div>
              ...
              <div className="flex items-center mt-1">
                ...
                <span className="text-gray-800">{currentWork.completionRate}%</span>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {statusOptions.map((option) => (
                <Button
                  key={option.value}
                  variant={currentWork.status === option.value ? 'primary' : 'outline'}
                  size="sm"
                  onClick={() => handleStatusChange(option.value)}
                  disabled={currentWork.status === option.value}
                  className={currentWork.status === option.value 
      ...
      <div className="grid grid-cols-1 gap-6 mb-6">
        <Card className="bg-white shadow-md rounded-lg p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium text-gray-800">일일 작업 보고서</h3>
            <Button 
              variant="primary" 
              size="sm" 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="bg-white shadow-md rounded-lg p-6">
          <h3 className="text-lg font-medium mb-4 text-gray-800">사용 자재</h3>
          <Table
      ...
      >
        ...
        <p className="text-sm text-gray-500 mt-2">
          이 작업은 되돌릴 수 없으며, 관련된 모든 보고서 데이터도 함께 삭제됩니다.
        </p>
      </Modal>
