...
const defaultData = {
  ...
  items: [
    ...
    {
      code: "3101051",
      name: "건축-수장-도배(3101051)",
      facility: "다가구매입임대(강북구) 0121 - 0502",
      work: "실크벽지 도배바름(합판.석고보드면)-철거포함,초배지미시공",
      spec: "벽면",
      unit: "M2",
      quantity: 1,
      note: "건축No.9",
...
export const QuantityCalculationDocument = ({ data = {} }) => {
  // 안전하게 데이터 접근하기 위해 처리
  const safeData = {
    ...
    <Document>
      <Page size="A4" orientation="landscape" style={styles.landscapePage}>
        <View style={styles.container}>
          {/* 제목 */}
          <Text style={styles.title}>물 량 산 출 근 거</Text>
          ...
          <View style={styles.table}>
            ...
            <View style={styles.tableHeaderRow}>
              ...
              <View style={[styles.tableHeader, styles.col20]}>
                <Text>작 업 명</Text>
              </View>
              <View style={[styles.tableHeader, styles.col10]}>
                <Text>규 격</Text>
              ...
              <View style={[styles.tableHeader, styles.col5, { width: "5%" }]}>
                <Text>수 량</Text>
              </View>
              <View
            ...
            {Array.isArray(safeData.items) && safeData.items.length > 0 ? (
              safeData.items.map((item, index) => (
                <View style={styles.tableRow} key={`item-${index}`}>
                  ...
                  <View style={[styles.tableCellLeft, styles.col15]}>
                    <Text>{item?.name || ""}</Text>
                  </View>
                  <View style={[styles.tableCellLeft, styles.col25]}>
                    <Text>{item?.facility || ""}</Text>
                  </View>
                  <View style={[styles.tableCellLeft, styles.col20]}>
                    <Text>{item?.work || ""}</Text>
                  </View>
                  <View style={[styles.tableCell, styles.col10]}>
                    <Text>{item?.spec || ""}</Text>
...
// 메인 컴포넌트
const QuantityCalculationPDF = ({ data, onClose }) => {
  ...
  const generatePDF = async () => {
    try {
      const blob = await pdf(
        <QuantityCalculationDocument data={data || defaultData} />
      ).toBlob();

      saveAs(blob, "물량산출근거.pdf");
    } catch (error) {
